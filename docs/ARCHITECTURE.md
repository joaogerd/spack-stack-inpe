...
```text
generic workflow logic must not contain machine-specific assumptions
machine-specific assumptions must live under a site directory or a site script
```
...
Stores environment templates that are copied into the JCSDA `spack-stack/envs/` directory before concretization and installation.

For the current JACI workflow:
...
Stores generic shell helpers used by several workflow scripts. These helpers load site configuration, initialize common paths and activate the selected `spack-stack` environment.
...
copy selected site files into the spack-stack tree
copy selected environment template into spack-stack/envs/
patch the install root for the current run
source the selected site setup.sh
activate the selected Spack environment
...
site-specific environment names
...
## Design limits for this first refactor
...
The goal of this first step is to separate concepts cleanly while preserving the current manual JACI workflow.
